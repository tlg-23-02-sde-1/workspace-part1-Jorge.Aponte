...
class Client {
    ...
    private static void passByValue() {
        int x = 4;
        System.out.println("Client: before the call, x is " + x);       // 4 or 8? 4

        // create Instructor and tell it to double my value
        Instructor instructor = new Instructor();
        instructor.doubleIt(x);                                         // prints 8 on the method doubleIt

        System.out.println("Client: after the call, x is " + x);        // 4 or 8? 4
    }
    ...
    private static void passByReference() {
        ...
        System.out.println("Client: after the call, the marker is " + m1.getColor());   // blue
    }
}